...
class OSMMapQualityEnvironment(Environment):
    """OpenStreetMap data quality checking environment."""
    ...
    def _apply_action(self, action: MapAction):
        ...
        if action.action_type == "set_tag":
            ...
            return -0.1, f"Cannot mark complete: {len(self._remaining_fixes)} issue(s) remain."

        return -0.1, f"Unknown action_type: '{action.action_type}'."

    def _check_fix_set_tag(self, key: str, value: str):
        fixed = False
        new_remaining = []
        ...
        return 0.05, f"Tag '{key}'='{value}' set (no matching required fix)."

    def _check_fix_coordinates(self, lat: float, lon: float):
        fixed = False
        new_remaining = []
        for fix in self._remaining_fixes:
            if fix["type"] != "fix_coordinates":
                new_remaining.append(fix)
                continue
            lat_ok = fix["lat_range"][0] <= lat <= fix["lat_range"][1]
            lon_ok = fix["lon_range"][0] <= lon <= fix["lon_range"][1]
        ...
        if fixed:
            self._remaining_fixes = new_remaining
            self._state.issues_fixed += 1
            return 0.3, f"Coordinates ({lat}, {lon}) accepted. +0.30"
        return -0.1, f"Coordinates ({lat}, {lon}) out of expected range."

    def _terminal_obs(self, feedback: str) -> MapObservation:
        return MapObservation(
